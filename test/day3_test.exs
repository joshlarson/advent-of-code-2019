defmodule Day3Test do
  ...
  describe "find_intersections" do
    test "includes only points that are in more than one path" do
      assert Day3.find_intersections("R2\nD2,R2,U2\n") |> Day3.includes_intersection?({0, 2}) ==
               false

      assert Day3.find_intersections("R2\nD2,R2,U2\n") |> Day3.includes_intersection?({1, 0}) ==
               false

      assert Day3.find_intersections("R2\nD2,R2,U2\n") |> Day3.includes_intersection?({2, 0}) ==
               true
    end

    test "does not include {0, 0}" do
      assert Day3.find_intersections("R1\nD1,R1,U1\n") |> Day3.includes_intersection?({0, 0}) ==
               false
    end
  end
  ...
  describe "closest_intersection" do
    ...
    test "works for example 2" do
      assert Day3.closest_intersection_distance(
  ...
  describe "closest_intersection_by_steps" do
    ...
    test "works for example 2" do
      assert Day3.closest_intersection_distance_by_steps(
               "R75,D30,R83,U83,L12,D49,R71,U7,L72\nU62,R66,U55,R34,D71,R55,D58,R83\n"
             ) == 610
    end
